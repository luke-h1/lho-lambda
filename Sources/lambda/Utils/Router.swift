...
final class Router: @unchecked Sendable {
        ...
        -> APIGatewayV2Response
    {
        ...
        var requestPath = event.rawPath.isEmpty ? event.context.http.path : event.rawPath
        
        // Strip /invoke prefix if present
        if requestPath.hasPrefix("/invoke") {
            requestPath = String(requestPath.dropFirst(7)) // Remove "/invoke"
            // If path is empty after stripping, set to root
            if requestPath.isEmpty {
                requestPath = "/"
            }
            // Ensure path starts with / if it doesn't after stripping
        ...
        context.logger.info(
            "Received request - Method: \(methodString), rawPath: '\(event.rawPath)', context.path: '\(event.context.http.path)', using path: '\(requestPath)'"
        )
...
func createRouter() -> Router {
    ...
    // GET /api/healthcheck
    router.add(method: .GET, path: "/api/healthcheck") { event, context in
